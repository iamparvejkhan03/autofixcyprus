            ...
            <Container className="flex flex-col gap-12 lg:gap-20 z-10 w-full h-full">
                <div className="flex flex-col lg:flex-row justify-between items-center gap-10 lg:gap-14 w-full">
                    <div className="lg:w-1/2 w-full px-2 flex flex-col items-start justify-between gap-5">
                        ...
                        <h1 className="text-5xl lg:text-6xl font-bold text-gray-900 leading-tight">{language === 'en' ? 'Fast ' : 'Γρήγορη '} <span className="text-blue-600">{language === 'en' ? 'Car Repair' : 'Επισκευή Αυτοκινήτου'}</span> - {language === 'en' ? 'Right On Time' : 'Στην Ώρα Του'}.</h1>
                        <p className="text-lg text-gray-600 leading-relaxed">{language === 'en' ? 'Professional car accident and body repair services across Cyprus. We handle everything from pickup to insurance support. Same-day service. Certified mechanics. 100% satisfaction guarantee.' : 'Επαγγελματικές υπηρεσίες επισκευής ατυχημάτων και φανοποιίας σε όλη την Κύπρο. Αναλαμβάνουμε τα πάντα: από την παραλαβή έως την υποστήριξη ασφάλειας. Εξυπηρέτηση αυθημερόν. Πιστοποιημένοι μηχανικοί. Εγγύηση 100% ικανοποίησης.'}</p>
                        ...
                        <div className="flex gap-4 flex-wrap">
                            <button onClick={() => navigate('/request-repair')} className={`inline-flex items-center justify-center whitespace-nowrap h-11 bg-blue-600 hover:bg-blue-700 text-white sm:px-8 ${language === 'gr' ? 'px-10' : 'px-4'} py-4 text-lg font-semibold rounded-md shadow-lg hover:shadow-xl  cursor-pointer`}>
                                {language === 'en' ? 'Book a Repair' : 'Κλείστε Επισκευή'}
